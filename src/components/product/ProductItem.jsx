import React from 'react';
import PropTypes from 'prop-types';
import Skeleton, { SkeletonTheme } from 'react-loading-skeleton';
import { useDispatch } from 'react-redux';

import { removeFromCart, addToCart } from 'actions/cartActions';
import { displayMoney, displayActionMessage } from 'helpers/utils';
import ImageLoader from '../ui/ImageLoader';

const ProductItem = ({
	product,
	onOpenModal,
	displaySelected,
	foundOnCart
}) => {
	const dispatch = useDispatch();

	const onClickItem = () => {
		if (product.id) {
			onOpenModal();
			displaySelected(product);
		}
	};

	const onAddToCart = () => {
		if (foundOnCart) {
			dispatch(removeFromCart(product.id));
			displayActionMessage('Item removed from cart', 'info');
		} else {
			dispatch(addToCart(product));
			displayActionMessage('Item added to cart', 'success');
		}
	};

	return (
		<SkeletonTheme color="#e1e1e1" highlightColor="#f2f2f2">
			<div
				className={`product-card ${!product.id ? 'product-loading' : ''}`}
				style={{
					border: foundOnCart ? '1px solid #cacaca' : '',
					boxShadow: foundOnCart ? '0 10px 15px rgba(0, 0, 0, .07)' : 'none'
				}}
			>
				{foundOnCart && <i className="fa fa-check product-card-check" />}
				<div
					className="product-card-content"
					onClick={onClickItem}
				>
					<div className="product-card-img-wrapper">
						{product.image ? (
							<ImageLoader
								className="product-card-img"
								src={product.image}
							/>
						) : <Skeleton width={'100%'} height={'90%'} />}
					</div>
					<div className="product-details">
						<h5 className="product-card-name text-overflow-ellipsis margin-auto">{product.name || <Skeleton width={80} />}</h5>
						<p className="product-card-brand">{product.brand || <Skeleton width={60} />}</p>
						<h4 className="product-card-price">{product.price ? displayMoney(product.price) : <Skeleton width={40} />}</h4>
					</div>
				</div>
				{product.id && (
					<button
						className={`product-card-button button-small button button-block ${foundOnCart ? 'button-border button-border-gray' : ''}`}
						onClick={onAddToCart}
					>
						{foundOnCart ? 'Remove from cart' : 'Add to cart'}
					</button>
				)}

			</div>
		</SkeletonTheme>
	);
};

ProductItem.propType = {
	onClickItem: PropTypes.func,
	dispatch: PropTypes.func.isRequired,
	product: PropTypes.object.isRequired,
	onOpenModal: PropTypes.func,
	foundOnCart: PropTypes.bool
};

export default ProductItem;
